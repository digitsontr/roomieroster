...
namespace RoommateMatcher.Hubs
{
	...
	public class ChatHub : Hub<IChatHub>
	{
		...
		public ChatHub(AppDbContext context, IMapper mapper, HubLogger logger, IEncryptionService encryptionService)
		{
			_context = context;
			_mapper = mapper;
			_logger = logger;
		...
		public async Task SendPrivate(string receiverName, string message)
		{
			try
			{
				message = await _encryptionService.EncryptAsync(message);
		...
		public async Task GetMessagesByChat(int chatId)
		{
			try
			{
				...
				var messageDtos = await Task.WhenAll(
						messages.Messages.Select(async message =>
						{
			...
			try
			{
				...
				foreach (var item in chats)
				{
					...
					userChats.Add(new ChatDto()
					{
						...
						LastMessage = await _encryptionService.DecryptAsync(item.Messages.LastOrDefault().Content),
